package org.committee.insCCP.externalDataLoaders;
...
public class MySqlLoader {
    ...
    static final String PASS = "Soviet1917";
    ...
    public static final String STANDARD_INSERT_STRING = "insert into userinfo values(\"%s\",\"%s\",\"%s\");";
	...
	public void selfCheck() {
		...
		try {
			this.statement.executeQuery("use insCCP;");
	    } catch(SQLException se) {
	    	if(se.getErrorCode() == 1049) isExist = false;
	    }
		
		if(!isExist) {
			try {
				this.statement.executeQuery("create database insCCP;");
		    } catch(SQLException se) {
		    	se.printStackTrace();
		    }
		}
	...
	public void loadUsers(Map<String, User> dst) {
		...
		try {
	        ...
	        while(rs.next()) dst.put(rs.getString("id"), new User(rs.getString("id"), rs.getString("password"), rs.getString("nickName")));
	...
	public ArrayList<String> searchFriend(String srcId) {
		...
		try {
	        ...
	        while(rs.next()) res.add(rs.getString("dstid"));
	...
	public ArrayList<String> searchApplyFriend(String srcId) {
		...
		try {
	        ...
	        while(rs.next()) res.add(rs.getString("srcid"));
